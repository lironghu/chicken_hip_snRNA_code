...
orth <- read.csv("/projects/0.snRNA/05.one_to_one_orthologs/AAA.One_to_One_Orthologs.RefMouse_modified2.csv")

#-----------------------------------------------------------matrix
data <- readRDS("/projects/chicken_hip/expression_matrix_new2/chicken_hip_scRNA_harmony_annotation_8samples_final.rds")

Idents(data) <- "Group"
data<- subset(data,idents="RJF")
...
counts[1:4,1:4]
write.csv(counts,file = "RJF_counts.csv")

metadata <- data@meta.data
write.csv(metadata, file = "RJF_metadata.csv", row.names = TRUE)