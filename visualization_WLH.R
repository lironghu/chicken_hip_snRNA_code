...
#-----------------------------------------------------------regulons
loom <- open_loom('WLH_SCENIC.loom')
...
write.csv(regulon_df, "WLH_regulons_TF_targets.csv", row.names = FALSE)

#-----------------------------------------------------------matrix
# Read metadata
metadata <- read.csv("WLH_metadata.csv", 
                     header = TRUE,
...
# Save results
write.csv(activity_df, "WLH_regulon_pro_by_celltype.csv", row.names = TRUE)
